...
async def send_chat_message(request: ChatRequest, user: User = Depends(get_authorized_user)):
    try:
        ...
        result = analyzer_service.process_query(
            ...
            structured_intent={"user_role": user.role, "session_id": request.session_id}
        )
        ...
        save_message(request.session_id, "assistant", summary_text, raw_data=raw_payload)
